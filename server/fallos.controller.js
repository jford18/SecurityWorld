...
const FALLAS_TECNICAS_EXPORT_HEADERS = [
  ...
  "ESTADO",
  "NOMBRE DE CONSOLA",
  "DURACION TOTAL DEL FALLO (SEG)",
  "N° DE MODIFICACIONES",
  "FECHA Y HORA DE MODIFICACION",
  "DURACION ENTRE 2 MODIFICACIONES (SEG)",
  "DURACION ENTRE 2 MODIFICACIONES (H)",
  "DEPARTAMENTO RESPONSABLE",
  "DETALLE DE NOVEDAD DETECTADA",
  "USUARIO QUE EDITA EL FALLO",
  "RESPONSABLE INICIAL",
  "ULTIMO USUARIO QUE EDITO",
  "DEPARTAMENTO RESPONSABLE (FINAL)",
];
...
export const exportFallosTecnicosConsultasExcel = async (req, res) => {
  ...
  try {
    ...
    const selectColumns = [
      ...
      `${buildDurationSql(duracionTotalFalloSegExpression)} AS "DURACION TOTAL DEL FALLO (H)"`,
      `CASE
      ...
      `COALESCE(C.NOMBRE_COMPLETO, C.NOMBRE_USUARIO) AS "USUARIO QUE EDITA EL FALLO"`,
      `COALESCE(D.NOMBRE_COMPLETO, D.NOMBRE_USUARIO) AS "RESPONSABLE INICIAL"`,
      `COALESCE(C.NOMBRE_COMPLETO, C.NOMBRE_USUARIO) AS "ULTIMO USUARIO QUE EDITO"`,
      `A.DEPARTAMENTO_ID AS "DEPARTAMENTO RESPONSABLE (FINAL)"`,
    ];
    ...
    const query = `
      ...
      LEFT JOIN USUARIOS C ON (C.ID = B.ULTIMO_USUARIO_EDITO_ID)
