...
EVENTS AS (
    SELECT
        A.DEVICE_CODE,
        COALESCE(A.SITE_NAME, E.SITE_NAME) AS SITE_NAME,
        A.CHANGED_AT AS OFFLINE_START_AT,
        (
            SELECT MIN(B.CHANGED_AT)
            FROM PUBLIC.HIK_CAMERA_RESOURCE_STATUS_LOG B
            WHERE B.DEVICE_CODE = A.DEVICE_CODE
              AND B.ONLINE_STATUS_NEW = 'ONLINE'
              AND B.CHANGED_AT > A.CHANGED_AT
        ) AS OFFLINE_END_AT
    FROM PUBLIC.HIK_CAMERA_RESOURCE_STATUS_LOG A
    JOIN PARAMS D ON (1 = 1)
    LEFT JOIN PUBLIC.HIK_CAMERA_RESOURCE_STATUS E ON (E.DEVICE_CODE = A.DEVICE_CODE)
    WHERE A.ONLINE_STATUS_NEW = 'OFFLINE'
      AND A.CHANGED_AT < D.TO_TS
),
DOWNTIME AS (
    SELECT
        SUM(
    ...
    LEFT JOIN PUBLIC.HACIENDA C ON (C.ID = B.HACIENDA_ID)
    JOIN PARAMS D ON (1 = 1)
    WHERE COALESCE(A.OFFLINE_END_AT, NOW()::TIMESTAMP) > D.FROM_TS
      AND (D.HACIENDA_ID IS NULL OR C.ID = D.HACIENDA_ID)
),
KPI AS (
    SELECT
...
OFFLINE AS (
    ...
    FROM OFFLINE_RAW A
    JOIN PARAMS B ON (1 = 1)
    WHERE COALESCE(A.OFFLINE_END_AT, NOW()::TIMESTAMP) > B.FROM_TS
),
AGR AS (
    SELECT
