...
WITH BASE AS (
    SELECT
        ...
        (COALESCE(A.TIPO_AFECTACION, '') || ' ' || COALESCE(B.DESCRIPCION, 'Sin problema')) AS PROBLEMA_LABEL,
        CASE
...
PROBLEMA_EJE AS (
    SELECT
        A.PROBLEMA_LABEL,
        STRING_AGG(DISTINCT A.NOMBRE_EQUIPO, ', ' ORDER BY A.NOMBRE_EQUIPO)
            FILTER (WHERE A.NOMBRE_EQUIPO <> 'Sin información') AS EQUIPOS_EJE
    FROM BASE A
    WHERE A.ES_PENDIENTE = 1
    GROUP BY A.PROBLEMA_LABEL
),
PEND_PROBLEMA_HACIENDA AS (
    SELECT
        ...
        CASE
            WHEN COALESCE(B.EQUIPOS_EJE, '') <> '' THEN B.EQUIPOS_EJE
            ELSE A.PROBLEMA_LABEL
        END AS EJE_LABEL,
        CASE
            WHEN COALESCE(B.EQUIPOS_EJE, '') <> '' THEN 1
            ELSE 0
        END AS TIENE_EQUIPO
    FROM BASE A
    LEFT JOIN PROBLEMA_EJE B ON (B.PROBLEMA_LABEL = A.PROBLEMA_LABEL)
    WHERE A.ES_PENDIENTE = 1
    GROUP BY A.PROBLEMA_LABEL, A.HACIENDA, B.EQUIPOS_EJE
),
